    ...
    else if (sdf->HasElement("sphericalCoordinates"))
    {
        ...
        if (world)
        {
                ...
                spherical_coordinates->HasElement("elevation"))
            {
                ...
                world->SetMagneticField(magnetic_field);
                ROS_INFO_STREAM("Magnetic field: " << int(magnetic_field.X() * 1e9) << "E\t"
                                                   << int(magnetic_field.Y() * 1e9) << "N\t"
                                                   << int(magnetic_field.Z() * 1e9) << "D");
            }
            else
            {
                ROS_WARN_STREAM(
                    "sphericalCoordinates ignored, all three parameters (latitudeDeg, longitudeDeg, elevation) must be "
                    "set");
            }
        }
    }
