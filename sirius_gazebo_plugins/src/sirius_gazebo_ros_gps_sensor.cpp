...
bool GazeboRosGpsSensor::LoadParameters()
{
    ...
    if (sdf->HasElement("positionCovariance"))
    {
        ...
        ROS_INFO_STREAM("Position covariance: " << position_covariance[0] << " " << position_covariance[1] << " "
    ...
    if (sdf->HasElement("positionCovarianceType"))
    {
        ...
        std::transform(position_covariance_type.begin(),
                       position_covariance_type.end(),
                       position_covariance_type.begin(),
        ...
        {
            gps_msg.position_covariance_type = 0;
            ROS_WARN_STREAM("missing or unknown positionCovarianceType: " << position_covariance_type
                                                                          << ". Set to UNKNOWN");
        }
    }
    ...
    if (sdf->HasElement("sphericalCoordinates"))
    {
        ...
        if (world)
        {
            ...
            {   
                ...
                this->spherical_offset = world->SphericalCoords()->SphericalFromLocal(
                                             ignition::math::Vector3d(-gazebo_origin.X(), -gazebo_origin.Y(), 0.0)) -
                                         world->SphericalCoords()->SphericalFromLocal(ignition::math::Vector3d::Zero);
